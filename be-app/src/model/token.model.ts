...
const TokenModel = {
	...
	createRefreshToken: async (refreshTokenValue: RefreshTokenValue) => {
		const { user_id, token_hash, jti, ip_address, user_agent } =
			refreshTokenValue;
	...
	updateRefreshToken: async (refreshTokenId: string, newJti: string) => {
	...
	revokeUserTokens: async (tokenHash: string) => {
		const revoked_at = new Date();
